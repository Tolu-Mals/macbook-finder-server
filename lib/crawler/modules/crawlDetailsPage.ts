  ...
  const detailsCrawler = new Crawler({
    ...
    callback: (error: Error, res: CrawlerRequestResponse, done: () => void) => {
      ...
      if (error) {
        ...
        const sellerScoreStr = $(sellerInfoGroup).find(SELLER_SCORE_SELECTOR).text().replace("%", "")
        const sellerScore = sellerScoreStr ? Number.parseInt($(sellerInfoGroup).find(SELLER_SCORE_SELECTOR).text().replace("%", "")) : undefined;
