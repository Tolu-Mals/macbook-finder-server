import type { CrawlerRequestOptions } from "crawler";
...
const crawlerCallbackFn: CrawlerRequestOptions["callback"] = (
	error,
	res,
	done,
) => {
	...
	if (nextPagePath) {
		...
		console.log("details: ", macbookDetailCrawlConfigs.length);

		if (macbookDetailCrawlConfigs.length > 0) {
			crawlDetailsPage(
				crawlerState,
				currentMacbooks,
				macbookDetailCrawlConfigs,
			);
		}
	}
