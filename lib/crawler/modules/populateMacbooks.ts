...
const populateMacbooks = ($: cheerio.CheerioAPI, productList: cheerio.Cheerio, macbooks: IMacbook[]) => {
  productList.each(function (_, product) {
    ...
    const price = convertNairaStringToFloat($(product).find(PRICE_SELECTOR).text())
